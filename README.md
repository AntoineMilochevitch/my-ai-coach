...
├── supabase/migrations/        # schéma SQL (0001 → 0017)
...
| `RESEND_API_KEY` | serveur (secret) | clé API Resend (envoi des e-mails d'auth) |
| `RESEND_FROM` | serveur | expéditeur, ex. `my-ai-coach <noreply@ton-domaine.com>` |
| `SEND_EMAIL_HOOK_SECRET` | serveur (secret) | secret du hook Supabase « Send Email » (`v1,whsec_…`) |

Les valeurs publiques (`VITE_*`, `SUPABASE_URL`) sont exclues du scan de secrets Netlify
via `SECRETS_SCAN_OMIT_KEYS` (`netlify.toml`).

### E-mails d'authentification (Resend)

Les e-mails d'auth (confirmation d'inscription, réinitialisation, magic link…) sont rendus au
thème du site et envoyés via **Resend**, grâce au **Send Email Hook** de Supabase Auth :

1. **Resend** : créer une clé API ; pour un vrai expéditeur, vérifier un domaine (sinon
   `onboarding@resend.dev` n'envoie qu'à l'adresse du compte, pour tester). Renseigner
   `RESEND_API_KEY` et `RESEND_FROM` côté Netlify.
2. **Supabase** → *Authentication → Hooks → Send Email Hook* : activer, type **HTTPS**, URL =
   `https://<ton-site>/.netlify/functions/send-auth-email`, **générer un secret** et le copier
   dans `SEND_EMAIL_HOOK_SECRET` (Netlify).
3. Le gabarit vit dans le code : [`_shared/auth-email.ts`](netlify/functions/_shared/auth-email.ts)
   (rendu) et [`send-auth-email.mts`](netlify/functions/send-auth-email.mts) (vérif. signature +
   envoi). Une fois le hook actif, Supabase délègue **tous** les e-mails d'auth à cette fonction.
...
Le schéma est versionné dans `supabase/migrations/` (`0001` → `0017`). Les migrations
